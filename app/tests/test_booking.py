...
def test_create_booking_uses_authenticated_owner(client, booking_seed, db_session):
    client.app.dependency_overrides[get_current_user] = lambda: booking_seed["user"]

    response = client.post(
        "/bookings/",
        json={
            "listing_id": str(booking_seed["listing"].id),
            "travel_date": "2026-04-15T09:00:00",
            "travel_count": 2,
    ...
    assert response.status_code == 201
    payload = response.json()
    assert payload["user_id"] == str(booking_seed["user"].id)
    assert payload["listing_id"] == str(booking_seed["listing"].id)
    assert payload["travel_count"] == 2
    assert payload["total_price_minor"] == 35000
...
def test_booking_stats_uses_model_first_aggregate_field_names(client, booking_seed):
    ...
    create_response = client.post(
        ...
        json={
            "listing_id": str(booking_seed["listing"].id),
            "travel_date": "2026-04-20T09:00:00",
            "travel_count": 1,
