...
class AdminDashboardService:
    ...
    def create_listing(self, category: str, payload: dict) -> dict:
        ...
        nested_stay_payload = self._extract_stay_nested_payload(payload) if category == "stay" else None
        listing = self.listings.create_listing(self._listing_model_data(category, payload))

        if nested_stay_payload is not None:
            self._persist_stay_nested_collections(listing.id, nested_stay_payload)
            listing = self.listings.get_listing(listing.id)
    ...
    def update_listing(self, category: str, listing_id: UUID, payload: dict) -> dict:
        ...
        if category == "stay":
            self._apply_stay_nested_updates_if_present(listing.id, payload)
    ...
    def _extract_stay_nested_payload(self, payload: dict) -> dict:
        return {
            "rooms": payload.get("rooms", []),
            "reviewMetrics": payload.get("reviewMetrics", []),
            "guestReviews": payload.get("guestReviews", []),
        }

    def _persist_stay_nested_collections(self, listing_id: UUID, payload: dict) -> None:
        self.listings.replace_rooms(listing_id, payload.get("rooms", []))
        self.listings.replace_review_metrics(listing_id, payload.get("reviewMetrics", []))
        self.listings.replace_guest_reviews(listing_id, payload.get("guestReviews", []))

    def _apply_stay_nested_updates_if_present(self, listing_id: UUID, payload: dict) -> None:
        if "rooms" in payload:
            self.listings.replace_rooms(listing_id, payload.get("rooms") or [])

        if "reviewMetrics" in payload:
            self.listings.replace_review_metrics(listing_id, payload.get("reviewMetrics") or [])

        if "guestReviews" in payload:
            self.listings.replace_guest_reviews(listing_id, payload.get("guestReviews") or [])
